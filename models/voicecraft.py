...
class VoiceCraft(
    ...
    def prepare_mask_intervals(self, y_lens):
        ...
        n_spans = 2
        mask_intervals = [[(9, 128), (134, 146)]]
        non_mask_intervals = [[(0, 9), (128, 134), (146, 188)]]
        """
        mask_intervals = []
        non_mask_intervals = []

        for i, y_len in enumerate(y_lens):
            # 确定 n_spans，也就是遮蔽区间的数量，根据均匀分布或者泊松分布生成
            if self.args.mask_sample_dist == "uniform":
                ...
                param = float(self.args.mask_sample_dist[len("poisson"):])  # 根据设置，为 1.0
                poisson_sample = torch.poisson(torch.tensor([param]))
                n_spans = int(poisson_sample.clamp(1, self.args.max_n_spans).item())  # max_n_spans = 3，应该是最大遮蔽区间，最后生成为1，2，3任意一个

            # 在 [1, y_len-1-self.args.mask_len_min] 范围内随机采样 n_spans 个起始位置 starts，并对其进行排序
            # mask_len_min = 1，这样设置确保不会超出边界
            starts = random.sample(range(1, y_len-1-self.args.mask_len_min), n_spans)
            starts = sorted(starts)

            # 检查相邻的起始位置是否过于接近（小于最小间隔 self.args.min_gap），如果是，则删除后面的起始位置。
            ...
            # 计算遮蔽区间
            temp_starts =  starts + [y_len]     # 加上句子的长度
    ...
    def rearrange(self, y, non_mask_intervals, mask_intervals):
        reduced_eog = getattr(self.args, "reduced_eog", 0)  # 设置为 1，意思是使用简化版本的 EOG
        rearranged_y = []
        for i in range(len(y)):
            if self.args.eos > 0:   # 2051
                assert reduced_eog
                # 对于所有非遮蔽区间（除了最后一个），直接截取相应片段
                # 最后一个非遮蔽区间后面添加 EOS 标记，这里最后一个非遮蔽区间一定是音频最后一个分段（包含结尾的分段）
                # 所有遮蔽区间后面添加 EOG 标记
                cur_y = [y[i, :, item[0]: item[1]] for item in non_mask_intervals[i][:-1]] + \
                        [torch.cat([y[i, :, non_mask_intervals[i][-1][0]: non_mask_intervals[i][-1][1]], self.eos], dim=-1)] + \
                        [torch.cat([y[i, :, item[0]: item[1]], self.eog], dim=-1) for item in mask_intervals[i]] # only insert eog to the last non-mask-interval, which is when the utterance actual ends
            else:
                if reduced_eog:
    ...
    def shift(self, rearranged_y):
        shifted_y = []
        patterns = []
        for i in range(len(rearranged_y)):  # rearranged_y -> [1,1,tensor(K,T)]
            # cur_y -> [K,T], tensor
            cur_patterns = [self.pattern.get_pattern(cur_y.shape[1]) for cur_y in rearranged_y[i]]
            out = [
                cur_pattern.build_pattern_sequence(
                    z=cur_y.unsqueeze(0).contiguous(),          # [1,K,T]，.contiguous() 方法用于确保张量（tensor）在内存中是连续存储的。
                    special_token=self.args.empty_token,        # 2048
                    keep_only_valid_steps=False                 # 是否只保留有效时间步
                    )
                for cur_pattern, cur_y in zip(cur_patterns, rearranged_y[i])
                ] # -> [values(1, 4, 192), indexes(4, 192), mask(4, 192)], 输出已存储文件
            # print_all_and_exit(cur_patterns = cur_patterns, out = out)
        ...
        # print_all_and_exit(True, shifted_y = shifted_y, patterns = patterns)
    ...
    def prepare_input_target(self, y, y_lens):
        ...
        # 选择部分将其遮蔽，使模型去学习上下文信息。具体输出参考函数内部说明
        ...
        # 重新排列 y，指定对应的遮蔽区间和非遮蔽区间，并在分段后加 eos 或 eog
        # 此时的顺序变为：y 的非遮蔽区间（最后衔接 eos）+ y 的遮蔽区间（每个遮蔽区间结尾有 eog）
    ...
    def forward(self, batch):
        ...
        Args:
          ...
          y_lens:
            ...
            before padding.
        """
        # print_all_and_exit(True, batch=batch)
        ...
        # makes attention mask and padding mask for x，获取文本 token 的注意力掩码和填充掩码
        ...
        # print_all_and_exit(False, y=y, y_lens=y_lens)
        # 准备输入
        y_input, new_y_lens, targets, y_padding_mask, y_attention_mask, mask_position, patterns = self.prepare_input_target(y, y_lens)
        # print_all_and_exit(False, y_input=y_input, new_y_lens=new_y_lens, targets=targets)
        # 解码器前向传播
        ...
        # print_all_and_exit(False, y_out=y_out)
        ...
        # print_all_and_exit(False, logits_former=logits)

        # 移除被遮蔽的位置
        logits_use = self.remove_mask(logits, mask_position, new_y_lens)
        # print_all_and_exit(False, logits_use = logits_use)
        
        # 移除 pattern shift，恢复原始顺序
        ...
        # print_all_and_exit(False, logits_final=logits_final, logit_masks=logit_masks)
    ...
    def inference_tts_batch(
        self,
        x: torch.Tensor,            # 合成目标文本 token 序列        
        x_lens: torch.Tensor,       # 合成目标文本 token 序列长度
        y: torch.Tensor,            # 提示音频离散 token 序列（多码本）
        top_k: int=-100,
        top_p: float=1.0,
        temperature: float=1.0,
        stop_repetition: int=3,
        ...
        *kargs
        ...
        Args:
          x:
            A 2-D tensor of shape (1, L). 文本 token 序列，单码本
          x_lens:
            A 1-D tensor of shape (1,). It contains the number of tokens in `x`
            before padding.
          y:
            A 3-D tensor of shape (1, T, K). prompt 音频 token 序列，多码本（K）
          top_k: (`optional`) int
            The number of highest probability tokens to keep for top-k-filtering. Default to -100.
          top_p: (`optional`) float
            For Neucleus sampling
        ...
        assert x_lens.ndim == 1, x_lens.shape
        assert y.ndim == 3, y.shape
        if self.args.special_first:             # 有特殊前缀，整体范围前移, False(0)
            y = y + int(self.args.n_special) 
        y = y.transpose(2,1) # [1,T,K] -> [1,K,T]       # 更新一下维度，先码本，后时轴
        assert y.shape[0] == 1 and y.shape[1] == self.args.n_codebooks, y.shape # there is no padding

        # make x attention mask and x_input，制作注意力掩码（上三角为真）和制作带有位置信息的目标文本嵌入
        x_attention_mask = torch.triu(torch.ones(x.shape[1], x.shape[1]), diagonal=1).bool().to(x.device)
        # x_attention_mask = torch.zeros(x.shape[1], x.shape[1]).bool().to(x.device)
        # 将文本 token 分别经过文本嵌入层和带有位置信息的嵌入层，制作文本嵌入
        x_input = self.text_embedding(x)
        x_input = self.text_positional_embedding(x_input)
        ...
        # replace tokens in y with the embeddings, add sum codebooks up
        # 经过 audio_embedding 模块的处理，将每个码本分别通过对应 embedding 模块，然后再堆积起来。
        ...
        # entering the generation stage
        # starting from line 708
        codebook_eog = [False] * self.args.n_codebooks                  # 每个码本是否结束
        generated = [] # doesn't contain any empty token, contain eog     生成的内容
        cur_generated = [[] for _ in range(batch_size)]                 # 当前已生成的内容
        # say 0 is empty, 4 is eog
        # tensor([[ 1,  2,  3,  4,  0,  0],
        #         [ 0,  1,  2,  3,  4,  0],
        #         [ 0,  0,  1,  2,  3,  4]])
        ...
        logging.info(f"silence tokens: {silence_tokens}, note that if you are not using the pretrained encodec 6f79c6a8, make sure you specified it yourself, rather than using the default")
        consec_silence_counts = [0 for _ in range(batch_size)]  # 每个采样的话连续的 silence token 计数
        prev_tokens = [None for _ in range(batch_size)] # 曾经的 token
        ##################### silence repetition handling #####################
        ##################### silence repetition handling #####################

        # prepare the cache placeholder
        # n_layers = 16, 2, bsz, num_heads, src_len, head_dim
        # decoder_layers = 16, x.shape[0](text batch size) = 1, kvcache = 1
        past = torch.ones([self.args.num_decoder_layers, 2, x.shape[0]], device=x.device, dtype=torch.float32) if kvcache else None  # -> [16,2,1]
        # logging.info(f"number of decoder layers: {self.args.num_decoder_layers}")
        # logging.info(f"number of decoder layers: {self.args.num_decoder_layers}")
        # logging.info(f"number of decoder layers: {self.args.num_decoder_layers}")
        ...
        def sample_helper(n_eog, logits, codebook_eog, top_k, top_p, temperature, prev_tokens, consec_silence_counts, stop_repetition, silence_tokens, cur_num_gen, keep):
            ...
            if n_eog == 0:
                ...
                for b in range(batch_size):
                    ...
                    # 检查是否有 EOG 标记被选中，如果有，则确定选中，并更新 codebook_eog 并记录应该保留的样本索引 keep，因此这里哪个样本先结束我们就保留哪个样本
                    if (
        ...
        while True:
            ...
            if cur_num_gen == 0:
                ...
                past = past.repeat(1, 1, batch_size) if past != None else None  # 是 kvcache 决定的缓存项，[16,2,bsz]
            else:
            ...
            # 前向传播，获得 y_out 和当前时间步缓存 present
            y_out, present = self.dec_forward(
            ...
            # 如果所有码本都还没结束，理论上 y_out 应该有 3 维，第一维度是 batch_size 
            if sum(codebook_eog) == 0:
                assert y_out.shape[0] == batch_size and y_out.ndim == 3, y_out.shape
            # 取最后一个 token
            # print_all_and_exit(True, y_out = y_out)
            y_out = y_out[:, -1:] # only take the last token
            logits = torch.stack([self.predict_layer[i](y_out) for i in range(self.args.n_codebooks)], dim=1) # [B K S card], S==1, so [B K 1 card]，因为只取了第一层
            ...
            # print_all_and_exit(False, logits_1 = logits)
            n_eog = sum(codebook_eog)
            # 确保结束符号的结束符号概率最小
            if self.args.eos > 0:
                for jj in range(self.args.n_codebooks):
            ...
            samples, codebook_eog, prev_tokens, consec_silence_counts, keep = \
                sample_helper(n_eog, logits, codebook_eog, top_k, top_p, temperature, prev_tokens, consec_silence_counts, stop_repetition, silence_tokens, cur_num_gen, keep)
            # print_all_and_exit(True, logits=logits)
            cur_num_gen += 1
            if cur_num_gen == 2:
                print_all_and_exit(True, samples=samples)
            # 此时还没有样本进入结束阶段，因此保存所有采样的样本生成内容
            if sum(codebook_eog) == 0: # no eog yet, keep batch_size of samples
                assert keep == None
                for b in range(batch_size):
                    cur_generated[b].append(samples[b].squeeze(-1))
            # 有样本进入了结束阶段，此时，只保留这个进入结束阶段的样本
            ...
            # samples.shape is [K,1] ? should be [B, K, 1]
            # ge samples_emb
            # 制作 samples 的嵌入
            ...
            # 如果生成结束了则将一些累计变量恢复出厂设置，然后跳出本轮生成循环
            if sum(codebook_eog) == self.args.n_codebooks: # generation for the current span is done
                codebook_eog = [False] * self.args.n_codebooks
                num_gen.append(cur_num_gen)
                cur_num_gen = 0
            ...
            # print_all_and_exit(False, embedded_y=embedded_y)
            embedded_y = torch.cat([embedded_y, samples_emb], dim=1)
            # print_all_and_exit(True, embedded_y=embedded_y)
            y_input = self.audio_positional_embedding(embedded_y) # [B T D]
            # make attention mask and padding mask
            y_attention_mask = torch.triu(torch.ones(y_input.shape[1], y_input.shape[1]), diagonal=1).bool().to(y.device)
    ...
    def logits_forward(
        ...
        x_lens = torch.LongTensor([x_encoded.shape[-1]]*(n_samples))
        x_attention_mask = torch.triu(torch.ones(x.shape[1], x.shape[1]), diagonal=1).bool().to(x.device)
        x_input = self.text_embedding(x)
        x_input = self.text_positional_embedding(x_input)
        ...
        # XXX: 这里直接走批量操作，不知道会不会有问题
        x_padding_mask = torch.full((n_samples, x_lens[0]), False).to(x.device)
